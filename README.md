...
- **视频标签复用**：只有一个视频标签（自动 Pin）；再点其他视频时，自动导航已有视频标签（不会越开越多）
- **回退即回主页**：关闭视频标签（或点 B站 logo 回主页），主页瞬间可见，状态零丢失
- **自动暂停**：点 logo 返回主页时，视频标签中的播放器自动暂停，不在后台继续出声
- **视频标签自动固定**：视频标签自动 Pin，切到其他网站时视频在后台继续播放（回主页时自动暂停）
- **双槽位（前台+后台）**：视频页右下角有「♪ 后台播放」按钮，把当前视频放进独立的后台标签（Pin、不聚焦）——前台继续刷视频，后台照常放音乐；回主页时只有前台视频暂停，后台播放不受影响
